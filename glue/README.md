...
```bash
glue$ make run
glue$ curl http://localhost:3000/hello
```
...
```bash
glue$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
glue$ aws cloudformation delete-stack --stack-name glue
```
...
```bash
glue$ pipenv lock -r > hello_world/requirements.txt
```
