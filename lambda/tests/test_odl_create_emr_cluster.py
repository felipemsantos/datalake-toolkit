...
lambda_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../odl_create_emr_cluster'))
...
with mock.patch.dict('os.environ', mock_vars):
    with mock.patch('boto3.client') as mock_boto3_client:
        with mock.patch('boto3.resource') as mock_boto3_resource:
            ...
            from odl_create_emr_cluster import lambda_handler

            def test_invoke_create_emr_cluster_with_ddb_exception():
                """
                Test the odl_create_emr_cluster function with DynamoDB exception
                :return:
                """

                error_response = {'Error': {'Code': 'MockErrorException'}}
                mock_boto3_resource.return_value.Table.return_value.scan.side_effect \
                    = ClientError(error_response, 'scan')
                mock_context = MockContext()
                mock_event = {}
                with pytest.raises(ClientError, match=r'.*(MockErrorException).*'):
                    lambda_handler(mock_event, mock_context)
                # Reseting side_effect for the next tests
                mock_boto3_resource.return_value.Table.return_value.scan.side_effect = None

            def test_invoke_create_emr_cluster():
                """
                Test the odl_create_emr_cluster function with valid event invocation
                :return:
                """
                mock_context = MockContext()
                mock_event = {}
                ...
                lambda_handler(mock_event, mock_context)
                mock_boto3_resource.return_value.Table.return_value.scan.return_value = None

            def test_invoke_create_emr_cluster_with_empty_stage_table():
                """
                Test the odl_create_emr_cluster function with valid event invocation
                :return:
                """
                mock_boto3_resource.reset_mock()
                mock_boto3_resource.return_value.Table.return_value.scan.return_value = {}
                mock_context = MockContext()
                mock_event = {}
                lambda_handler(mock_event, mock_context)

            def test_invoke_create_emr_cluster_with_existing_cluster():
                """
                Test the odl_create_emr_cluster function with valid event invocation and the cluster is already running
                :return:
                """
                mock_boto3_resource.return_value.Table.return_value.scan.return_value = {"Items": "mock"}
            ...
            def test_invoke_create_emr_cluster_run_job_flow_exception():
                """
                Test the odl_create_emr_cluster function with valid event invocation
                :return:
                """
                ...
                error_response = {'Error': {'Code': 'MockErrorException'}}
                mock_boto3_resource.return_value.Table.return_value.scan.return_value = {"Items": "mock"}
                mock_boto3_client.return_value.run_job_flow.side_effect = ClientError(error_response, 'run_job_flow')
                with pytest.raises(ClientError, match=r'.*(MockErrorException).*'):
                    lambda_handler(mock_event, mock_context)
                mock_boto3_client.return_value.run_job_flow.side_effect = None
