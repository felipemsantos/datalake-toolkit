...
lambda_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../odl_catalog_hive_metadata_es'))
...
mock_vars = {
    ...
    'DYNAMO_DB_CONTROL': 'mock-bigdata-OdlControl',
    'BUCKET_ATHENA_QUERY_OUTPUT': 'mock-bigdata-athena',
    'KEY_ATHENA_QUERY_OUTPUT': 'lambda_datalake_hive_catalog_es',
...
with mock.patch.dict('os.environ', mock_vars):
    with mock.patch('boto3.client') as mock_boto3_client:
        with mock.patch('boto3.resource') as mock_boto3_resource:
            ...
            # when the module is loaded import lambda_handler
            from odl_catalog_hive_metadata_es import lambda_handler

            def test_catalog_hive_metadata_es():
                """
                Test the odl_catalog_hive_metadata_es function
                :return:
                """
                mock_context = MockContext()
                mock_event = {}
                mock_boto3_client.return_value.get_query_execution.return_value = {
                    'QueryExecution': {
                        'Status': {
                            'State': 'SUCCEEDED'
                        }
                ...
                mock_boto3_client.return_value.get_query_execution.return_value = None
                mock_boto3_resource.reset_mock()
                mock_boto3_client.reset_mock()

            def test_catalog_hive_metadata_es_timeout():
                """
                Test the odl_catalog_hive_metadata_es function with query timeout (must wait 60 seconds)
                :return:
                """
                mock_context = MockContext()
                mock_event = {}
                ...
                with pytest.raises(Exception, match=r'.*(Unable to complete the Athena Query).*'):
                    lambda_handler(mock_event, mock_context)
                mock_boto3_resource.reset_mock()
                mock_boto3_client.reset_mock()
